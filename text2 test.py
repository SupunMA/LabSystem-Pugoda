...
def process_hl7_data(hl7_data, output_dir, result_counter):
    ...
    raw_filename = f"raw_hl7_log_{timestamp}_result{result_counter}.txt"
    ...
    try:
        ...
        print(f"❌ Error saving raw HL7: {e}")
    ...
    if parsed_results:
        ...
        try:
            save_to_csv(parsed_results.copy())
            print(f"\n💾 Results saved to CSV with {len(parsed_results)} fields")
        except Exception as e:
            print(f"❌ Error saving CSV: {e}")
        
        # Save parsed results to TXT file
